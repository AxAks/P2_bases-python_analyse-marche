...
def main():
    ...
    Elle copie ces informations dans un fichier CSV
    d'après la catégorie du livre
    et télécharge l'image de couverture du livre
    dans un dossier dédiée à sa catégorie
    """
    timestamp_start = datetime.now()
    site_url = 'http://books.toscrape.com/'
    relative_category_urls_list = scrape_site(site_url)
    absolute_category_urls_list = \
        get_absolute_category_urls_list(relative_category_urls_list)
    all_categories_pages_list = \
        get_category_pagination_pages(absolute_category_urls_list)
    ...
    relative_books_urls_list = \
        list_of_lists_to_flat_list(relative_books_urls_lists)
    absolute_books_urls_list = \
        reformat_list_of_relative_urls_to_absolute(relative_books_urls_list)
...
def scrape_site(site_url):
    ...
    relative_category_urls_list = [category_url['href']
...
def get_absolute_category_urls_list(relative_category_urls_list):
    """
    Cette fonction reformate les URLs relatives des categories
    en URLs absolues depuis la racine du site.
    """
    site_url = 'http://books.toscrape.com/'
    absolute_category_urls_list = [urljoin(site_url, relative_category_url)
                                   for relative_category_url
                                   in relative_category_urls_list]
    return absolute_category_urls_list


def get_category_pagination_pages(absolute_category_urls_list):
    """
    Cette fonction permet de vérifier s'il existe plusieurs pages
    pour une liste d'URLs de catégorie
    ...
    all_categories_pages = [get_all_pages_category(absolute_category_url)
                            for absolute_category_url
                            in absolute_category_urls_list]
