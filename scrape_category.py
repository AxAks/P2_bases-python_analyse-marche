...
def main():
    """
    Cette fonction récupère toutes les urls des livres
    pour une page catégorie donnée,
    en récupère les informations
    et les copie dans un fichier CSV pour cette catégorie
    Elle enregistre ensuite les images de couverture des livres
    ...
    absolute_books_urls = \
        reformat_list_of_relative_urls_to_absolute(relative_books_urls)
...
def get_all_pages_category(url):
    """
    Cette fonction vérifie s'il existe plusieurs pages
    ...
    print(f"Nombre de pages trouvées pour la catégorie :"
          f" {len(pagination_pages)}\n"
          f"---")
...
def get_books_urls(page):
    """
    Cette fonction récupère les URLs relatives des pages produit
    ...
    relative_books_urls = [{'titre': lien['title'], 'lien': lien['href']}
                           for lien in liens]
...
def get_category_books_urls(pagination_pages):
    """
    Cette fonction récupère les URLs relatives des pages produit
    de toutes les pages d'une catégorie
    et les ajoute à une liste.
    """
    relative_books_urls_list = [get_books_urls(page)
...
def reformat_list_of_relative_urls_to_absolute(relative_urls):
    """
    Cette fonction reformate une liste d'URLs relatives
    en liste d'URLs absolues depuis la racine du site.
    """
    absolute_urls_list = [urljoin("http://books.toscrape.com/catalogue/",
                                  relative_url['lien'].lstrip('../'))
...
def add_book_infos_to_list(absolute_books_urls):
    """
    Cette fonction récupère les informations de chaque livre
    pour une liste d'URLs produit sous forme de dictionnaire
    et les ajoute à une liste.
    """
    book_infos_list = [get_book_infos(absolute_book_url)
...
def write_csv_loop(book_infos_list):
    """
    Cette fonction effectue une boucle sur write_csv
    pour écrire un dictionnaire de données dans un fichier CSV.
    """
